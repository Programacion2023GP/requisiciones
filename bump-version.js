...
const bumpVersion = async () => {
   ...
   const bumpMajor = await askYesNo(
      "¿Incrementar versión MAYOR (cambios grandes o ruptura)? (y/n): ",
   );
   ...
   if (bumpMajor) {
      ...
      const bumpMinor = await askYesNo(
         "¿Incrementar versión MENOR (nuevas funciones)? (y/n): ",
      );
      if (bumpMinor) {
         minor++;
         patch = 0;
         ...
         const bumpPatch = await askYesNo(
            "¿Incrementar versión PATCH (correcciones menores)? (y/n): ",
         );
         if (bumpPatch) {
            patch++;
            build = 0;
   ...
   if (setStage) {
      const newStage = await ask(
         chalk.magentaBright("Indica el estado (beta / rc / prod): "),
      );
      if (
         ["beta", "rc", "prod", "production"].includes(newStage.toLowerCase())
      ) {
         stageLabel =
            newStage.toLowerCase() === "production"
               ? "prod"
               : newStage.toLowerCase();
      } else {
         console.log(
            chalk.red("⚠️ Estado no válido, se mantiene el anterior."),
         );
      }
   }
   ...
   for (const category of categories) {
      ...
      while (true) {
         ...
         const hasSub = await askYesNo(
            "¿Agregar subpuntos a este ítem? (y/n): ",
         );
         if (hasSub) {
            const sublist = [];
            while (true) {
...
`;
   ...
   console.log(
      chalk.blueBright("🗒️ Cambios registrados y organizados en versions.md\n"),
   );
