            ...
            <div className="relative z-0 w-full mb-5">
              <div className="flex items-center border-b-2 border-gray-200 focus-within:border-black">
                ...
                <button
                  type="button"
                  onClick={() =>
                    setFieldValue(name, Math.max((field.value || 0) - (decimals ? 0.1 : 1), min || 0))
                  }
                ...
                <button
                  type="button"
                  onClick={() =>
                    setFieldValue(name, Math.min((field.value || 0) + (decimals ? 0.1 : 1), max || Infinity))
                  }
                  className="px-3 py-2 text-gray-500 hover:text-black focus:outline-none"
                >
                  <FaPlus />
        ...
        {({ field, form: { errors, touched, values } }: any) => {
          ...
          return (
            <div className="relative z-0 w-full mb-5">
              ...
              >
                {showPassword ? <IoMdEye className="h-5 w-5" /> : <IoIosEyeOff className="h-5 w-5" />}
              </button>
              {error && (
                <span
                  className="text-sm font-semibold text-red-600"
...
export const FormikAutocomplete = <T extends Record<string, any>>({
  ...
  const handleClickOutside = (e: MouseEvent) => {
    if (
      inputRef.current && !inputRef.current.contains(e.target as Node) && // Si el clic no es en el input
      menuRef.current && !menuRef.current.contains(e.target as Node) // Si el clic no es en el menú de opciones
    ) {
  ...
  const selectOption = (option: T, setFieldValue: (name: string, value: any) => void) => {
        ...
        return (
          <div className="relative  w-full mb-5" >
            <input
              ...
              placeholder=" "
              value={
             Array.isArray(options) &&  options.find((item) => item[idKey] === values?.[name])?.[
                  labelKey
                ] || textSearch
              }
              onClick={() => {
