...
export const Agtable: React.FC<TypeTable> = ({
   ...
   const handleExportExcel = async () => {
      // Mostrar el modal con input
      const { value: fileName } = await Swal.fire({
         title: "Nombre del archivo",
         input: "text",
         inputLabel: "Ingresa el nombre para el archivo (sin extensión)",
         inputPlaceholder: "mi_exportacion",
      ...
      if (!fileName) {
         // Si canceló o no ingresó nada, no hacemos nada
         return;
      }
      ...
      // Ya que tenemos el nombre, hacemos la exportación
      const rowData: any[] = [];
      ...
      const cols = columnDefsH.map((col) => ({
   ...
   useEffect(() => {
      if (
         ...
         setFiltersColapseOptions((prev) => ({
            ...prev,
            titles: Object.keys(
               dataTable.data.length > 0 ? dataTable.data[0] : {},
            ), // Extrae las claves del primer elemento
         }));
      }
   ...
   const handleFilterChange = (params: {
      ...
      if (sqlWhereClause !== "") {
         const existingWhere = backUrl?.startSearchFilter?.where || "";

         const finalWhere = mergeWhereClauseAdvanced(
            existingWhere,
            sqlWhereClause,
         );
         ...
         const defaultWhere = backUrl?.startSearchFilter?.where;

         console.log(
            "No hay filtros, aplicando búsqueda inicial:",
            defaultWhere,
         );
         ...
         if (defaultWhere) {
            ...
            mutation.mutate({
               url: backUrl?.pathName ? backUrl.pathName : "",
               method: "POST",
               data: { sql: defaultWhere },
            });
         } else {
            // No hay filtros ni búsqueda inicial, resetea a vacío
            setDataTable((prev) => ({
               previous: prev.previous,
               data: [],
               sql: "",
   ...
   function mergeWhereClauseAdvanced(
      originalWhere: string,
      newCondition: string,
   ): string {
      if (!newCondition) return originalWhere || "";

      const newField = newCondition.trim().split(" ")[0]; // Ej: 'Ejercicio'
      const simplifiedField = newField.replace(/\W/g, ""); // Remueve símbolos

      if (!originalWhere) return newCondition;

      // Eliminar condiciones del mismo campo (solo condiciones simples)
      const regexSimple = new RegExp(
         `\\b${simplifiedField}\\b\\s*(=|!=|<>|<|>|<=|>=|LIKE|BETWEEN|IN)\\s*('[^']*'|%?'.*'?%?|\\d+)`,
         "gi",
      );
      let cleanedWhere = originalWhere.replace(regexSimple, "");

      // También limpiar AND sueltos o repetidos
                  ...
                  <Tooltip content="Exportar a Excel">
                     <div className="mr-4">
                        <Button
                           ...
                           color="green"
                           variant="solid">
                           <icons.Fa.FaFileExcel size={20} />
                        </Button>
                     </div>
                  </Tooltip>
                  {permissionsUserTable?.buttonElement ? (
                     <PermissionMenu
                        IdMenu={permissionsUserTable?.buttonElement}>
                        <div className="flex flex-row w-full space-x-2">
                           {buttonElement}
                        </div>
                     </PermissionMenu>
                  ) : (
                     <div className="flex-1">{buttonElement}</div>
                  )}
               </div>
            </div>
                     ...
                     <div className="flex items-stretch text-wrap">
                        <div className="flex-1">
                           <AgGridReact
                              ...
                              loading={mutation.status === "pending" || loading}
                                          ...
                                          <div className="space-y-2">
                                             ...
                                             {filteredEntries.map(
                                                ([value, count]) => (
                                                   <div
                                                      key={value}
                                                      className="flex items-center justify-between p-2 text-gray-700 border-b border-gray-300 cursor-pointer hover:text-purple-500 hover:font-semibold"
                                                      onClick={() => {
                                                         setSelectedFilter(
                                                            column.field
                                                               ? column.field
                                                      ...
                                                      <div className="px-2 py-1 text-xs text-gray-500 bg-purple-300 rounded-full">
                                                         {count as number}
                                                      </div>
                                                   </div>
                                                ),
                        ...
                        {columnDefsH.map((col) => (
                           <div
                              ...
                              <input
                                 ...
                                 className="w-5 h-5 mb-1 text-blue-600 transition duration-200 ease-in-out bg-gray-200 border-gray-300 rounded-lg cursor-pointer focus:ring-2 focus:ring-blue-500"
                              />
                              <label
                                 htmlFor={col.field}
